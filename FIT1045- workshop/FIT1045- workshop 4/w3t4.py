...
for i in range(len(listTableTide)):
    ...
    for j in range(len(listTableTide[i])):
        ...
        if lowest > listTableTide[i][j]:
            ...
            lowesti = i
            lowestj = j
            lowestT = listTableTime[lowesti][lowestj]
...
for i in range(len(lowestTime)):
    if lowestTime[i] > 12.00:
        lowestTime[i] = round((lowestTime[i] - 12.00), 2)
        past12 = str(lowestTime[i]) + ' PM'
        lowestTime[i] = past12
    else:
        pre12 = str(lowestTime[i]) + ' AM'
        lowestTime[i] = pre12
...
for i in range(len(listTableTide)):
    ...
    for j in range(len(listTableTide[i])):
        ...
        if highest < listTableTide[i][j]:
            ...
            highesti = i
            highestj = j
            highestT = listTableTime[highesti][highestj]
